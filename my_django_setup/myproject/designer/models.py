...
class ManufacturingPlan(models.Model):
    ...
    designer = models.ForeignKey(User, on_delete=models.SET_NULL, null=True, related_name='manufacturing_plans')
...
class ManufacturingStep(models.Model):
    ...
    plan = models.ForeignKey(ManufacturingPlan, on_delete=models.CASCADE, related_name='steps')
    name = models.CharField(max_length=200)
    description = models.TextField(blank=True)
...
class QualityChecklistItem(models.Model):
    ...
    step = models.ForeignKey(ManufacturingStep, on_delete=models.CASCADE, related_name='quality_checklist')
...
class StepMaterial(models.Model):
    step = models.ForeignKey(ManufacturingStep, on_delete=models.CASCADE, related_name='materials')
...
class StepDependency(models.Model):
    from_step = models.ForeignKey(
        ManufacturingStep, on_delete=models.CASCADE, related_name='outgoing_dependencies'
    )
    to_step = models.ForeignKey(
        ManufacturingStep, on_delete=models.CASCADE, related_name='incoming_dependencies'
    )
